...
#### ДоGодсистема Расчёт управленческой зарплаты
...
1. В режиме администратора настроить [телеграмм-бот](https://github.com/netology-code/fonecmid-diplom/blob/onec-mid-31/tasks/telegram.md)
...
С помощью обработки Заполнение графиков работ, заполните соответствующий график. 
2. Заполнить справочник Сотрудники, а так же регистр сведения УсловияОплатыСотрудников. 
...
1. Кадровик-расчетчик должен  в разделе сервис назначить график работ для отпуска (Должны учитываться все дни)

2. Для каждого сотрудника заполнить регистры сведений: СведенияОбОтпусках и УсловияОплатыСотрудников
...
Для тестирование необходимо скачать из корневой папки 4 файла для автотеста и 2 файла mxl.
Сохранить на компьютер файлы из репоитория Ноябрь.mxl и октябрь.mxl.
